...
<head>
    ...
    ?>
  <style>.timeline,.timeline .container,.timeline-group{position:relative}.timeline-date,h1{text-align:center}body{font-family:Arial,sans-serif;margin:0;padding:0;background-color:#f5f5f5;color:#333}.timeline-event h3,h1{color:#<?= $color_primary ?>}h1{margin:20px 0;font-size:1.8em}.timeline{max-width:700px;margin:0 auto;padding:20px}.timeline-group{margin-bottom:20px}.timeline-date,.timeline-event{margin-bottom:10px}.timeline-date{color: #<?= $color_primary ?>;padding:5px 10px;border-radius:5px;font-weight:700;display:inline-block; background: rgba(<?= hexToRgb($color_primary)?>, .05)}.timeline-events{background:#fff;border:1px solid #ddd;padding:10px 15px;border-radius:5px;box-shadow:0 2px 4px rgba(0,0,0,.1)}.timeline-event:last-child{margin-bottom:0}.timeline-event h3{font-size:1.2em;margin:0 0 5px}.timeline-event p{font-size:1em;margin:0;color:#555}</style>
</head>
<body>
    <section class="timeline">
        <h1>Próximas fechas de pago</h1>
        <p style="font-size: 20px;">Hola <b><?= $company->user_name ?></b>, ten presente las siguientes fechas para pagar tus impuestos.</p>
        <div class="container">
