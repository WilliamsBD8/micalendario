            ...
            <table class="table table-light table-sm table-borderless">
                <thead>
                    <tr class="">
                        ...
                        <th colspan="<?= count($last_digit_nit) + 1 ?>"  class="table-primary text-center">Presentación / Pago</th>
                    </tr>
                    <tr>
                        <th class="table-primary">Último dígito del NIT</th>
                        <?php foreach ($last_digit_nit as $key => $dig): ?>
                            <th class="table-primary"><?= $dig ?></th>
                        <?php endforeach ?>
                    </tr>
                </thead>
                <tbody class="table-border-bottom-0">
                    <?php foreach($data_aux as $tax): ?>
                        <tr>
                            ...
                            <?php foreach ($tax->details as $key => $dig): ?>
                                <td class="<?= $dig->last_digit_nit == session('filter')->last_dig ? 'table-info':'table-white' ?>"><?= date('d', strtotime($dig->date)) ?></td>
                            <?php endforeach ?>
                        </tr>
                    <?php endforeach ?>
                </tbody>
